...
arrowCheck.textContent = add(num1,num2) + ' | ' + sub(num1,num2) + ' | ' + mul(num1,num2) + ' | ' + div(num1,num2);
...
switch (item) {
    ...
    case 'Corn':
        ...
        switchCheck.style.color = yellow;
        break;
    default:
...
const mappedArray = scatteredInts.map((val) => {
    if (val > overUnder) return ` over`;
    else if (val < overUnder) return ` under`;
    else if (val === overUnder) return ` on the dot`;
});
...
const reduceArray = mixedArray.reduce((ac,cv,ci,ar) => {
    if (ci === ar.length-1) return ac += 'and ' + cv.toLowerCase() + '!';
    else return ac += cv.toLowerCase() + ', ';
},`Today we're serving `);
...
// "The first number is ${the first number of scatteredInts}, the second is ${the second number of scatteredInts},
...
function describeArray(n1,n2,...rest) {
    return `The first number is ${n1}, the second is ${n2}, and I don't know the rest but together it all kind of looks like
    ${rest.reduce((ac,cv) => ac += cv)}`;
}

restCheck.textContent = describeArray(...scatteredInts);
